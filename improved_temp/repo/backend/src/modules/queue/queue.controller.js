...
const { emitQueueUpdate } = require('../../realtime/socket');
const {
...
async function generateToken(req, res, next) {
  ...
  try {
    ...
    emitQueueUpdate(doctorId, dateStr, { type: 'token_generated', token });
...
async function updateCurrentToken(req, res, next) {
  ...
  try {
    ...
    emitQueueUpdate(doctorId, dateStr, { type: 'current_token_updated', ...updated });
...
async function resetDailyQueue(req, res, next) {
  ...
  try {
    ...
    emitQueueUpdate(doctorId, dateStr, { type: 'queue_reset', ...updated });
    return res.json(updated);
  } catch (err) {
    next(err);
  }
